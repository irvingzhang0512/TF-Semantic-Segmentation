...
def flip_dim(tensor_list, prob=0.5, dim=1):
    ...
    Note that tf.random_flip_left_right and tf.random_flip_up_down isn't
    used so that we can control for the probability as well as ensure the
...
def _image_dimensions(image, rank):
    ...
    Returns:
      A list of corresponding to the dimensions of the input image. Dimensions
        that are statically known are python integers, otherwise they are
        integer scalar tensors.
    """
    if image.get_shape().is_fully_defined():
        ...
        return [
            s if s is not None else d for s, d in zip(static_shape,
                                                      dynamic_shape)
        ]
...
def pad_to_bounding_box(image, offset_height, offset_width, target_height,
    ...
    with tf.compat.v1.name_scope(None, 'pad_to_bounding_box', [image]):
        ...
        if original_dtype != tf.float32 and original_dtype != tf.float64:
            # If image dtype is not float
            # we convert it to int32 to avoid overflow.
...
def _crop(image, offset_height, offset_width, crop_height, crop_width):
    ...
    Note that the method doesn't assume we know the input image size but
    it does assume we know the input image rank.
...
def resize_to_range(image,
    ...
    Args:
      ...
      max_size: (scalar) maximum allowed size of the larger image side. Note
        that the output dimension is no larger than max_size and may be
        slightly smaller than min_size when factor is not None.
      factor: Make output size multiple of factor plus one.
      align_corners: If True, exactly align all 4 corners of input and output.
      label_layout_is_chw: If true, the label has shape
        [channel, height, width]. We support this case because for some
        instance segmentation dataset, the instance segmentation is saved
        as [num_instances, height, width].
      scope: Optional name scope.
    ...
    with tf.compat.v1.name_scope(scope, 'resize_to_range', [image]):
        ...
        if max_size is not None:
            ...
            # make sure the max dimension after resizing is no larger
            # than max_size.
        ...
        new_size = large_size
        if max_size is not None:
            # Calculate the smaller of the possible sizes, use that if the
            # larger is too big.
