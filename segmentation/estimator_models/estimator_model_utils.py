...
__all__ = ['get_estimator_spec', 'get_preprocess_by_frontend',
           'preprocess_subtract_imagenet_mean', 'preprocess_zero_to_one',
           'preprocess_zero_to_one']


def get_estimator_spec(mode, logits, init_fn, labels,
                       num_classes=None, params=None, **kwargs):
    ...
    confusion_matrix = tf.confusion_matrix(
        valid_labels, valid_preds, num_classes=num_classes)
        ...
        if params['debug_mode']:
            tf.summary.image('images',
                             ...
                             tf.concat(axis=2,
                                       values=[
                                           tf.cast(
                                               kwargs['features']['image'],
                                               tf.uint8
                                           ),
                                           gt_color_labels,
                                           pred_color_labels]),
                             max_outputs=params['summary_image_max_number'])
...
def _get_train_op(params, total_loss):
    ...
    with tf.control_dependencies(update_ops):
        train_op = optimizer.minimize(
            total_loss, tf.train.get_global_step(), var_list=train_var_list)
    return train_op
