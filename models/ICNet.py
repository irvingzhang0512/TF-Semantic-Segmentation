...
def InterpBlock(net, level, feature_map_shape, pooling_type):
    ...
    kernel_size = [int(np.round(float(feature_map_shape[0]) / float(level))),
                   int(np.round(float(feature_map_shape[1]) / float(level)))]
...
def build_icnet(inputs, label_size, num_classes, preset_model='ICNet', pooling_type="MAX",
    ...
    if frontend == 'Res50':
        with slim.arg_scope(resnet_v2.resnet_arg_scope(weight_decay=weight_decay)):
            ...
            init_fn = slim.assign_from_checkpoint_fn(os.path.join(pretrained_dir, 'resnet_v2_50.ckpt'),
        ...
        with slim.arg_scope(resnet_v2.resnet_arg_scope(weight_decay=weight_decay)):
            ...
            # ICNet requires pre-trained ResNet weights
            init_fn = slim.assign_from_checkpoint_fn(os.path.join(pretrained_dir, 'resnet_v2_101.ckpt'),
        ...
        with slim.arg_scope(resnet_v2.resnet_arg_scope(weight_decay=weight_decay)):
            ...
            # ICNet requires pre-trained ResNet weights
            init_fn = slim.assign_from_checkpoint_fn(os.path.join(pretrained_dir, 'resnet_v2_152.ckpt'),
    ...
    block_32 = PyramidPoolingModule(end_points_32['pool3'], feature_map_shape=feature_map_shape,
