...
from estimator_models.estimator_model_utils import get_estimator_spec, \
    get_preprocess_by_frontend
...
def atrous_spatial_pyramid_pooling(inputs, output_stride,
    ...
    Args:
      ...
      output_stride: The ResNet unit's stride.
        Determines the rates for atrous convolution.
    ...
    with tf.variable_scope("aspp"):
        ...
        with tf.contrib.slim.arg_scope(resnet_v2.resnet_arg_scope(
            ...
            with arg_scope([layers.batch_norm], is_training=is_training):
                ...
                # (a) one 1x1 convolution and three 3x3 convolutions
                # with rates = (6, 12, 18) when output stride = 16.
                # the rates are doubled when output stride = 8.
                conv_1x1 = layers_lib.conv2d(
                    inputs, depth, [1, 1], stride=1, scope="conv_1x1")
                conv_3x3_1 = resnet_utils.conv2d_same(
                    inputs, depth, 3, stride=1, rate=atrous_rates[0],
                    scope='conv_3x3_1')
                conv_3x3_2 = resnet_utils.conv2d_same(
                    inputs, depth, 3, stride=1, rate=atrous_rates[1],
                    scope='conv_3x3_2')
                conv_3x3_3 = resnet_utils.conv2d_same(
                    inputs, depth, 3, stride=1, rate=atrous_rates[2],
                    scope='conv_3x3_3')
...
def deeplab_v3_generator(num_classes,
    ...
    Args:
      ...
      pretrained_dir: The path to the dir that contains pre-trained models.
      batch_norm_decay: The moving average decay when estimating layer
        activation statistics in batch normalization.
      weight_decay:
    ...
    def model(inputs, is_training):
        logits, end_points, frontend_scope, init_fn = frontend_builder.\
            build_frontend(
                inputs, frontend,
        ...
        with tf.contrib.slim.arg_scope(
            ...
            net = atrous_spatial_pyramid_pooling(
...
def get_deeplabv3_model_fn(num_classes,
    ...
    def deeplabv3_model_fn(features, labels, mode, params):
        _preprocess_fn = get_preprocess_by_frontend(frontend=frontend)
        preprocessed_image = _preprocess_fn(features['image'])
        ...
        logits, init_fn = network(
            preprocessed_image,
            (mode == tf.estimator.ModeKeys.TRAIN)
        )
        return get_estimator_spec(mode, logits, init_fn, labels=labels,
                                  num_classes=num_classes, params=params,
                                  features=features, )
